...
def test_basic_dataset_caching():
    with tempfile.TemporaryDirectory() as path:
        ...
        assert b['MR_T1_voxels'].shape == (1, 3, 256, 192)
...
def test_paired_h5_sampler_random():
    with tempfile.TemporaryDirectory() as path:
        ...
        context = {'dataset_h5_disable_sampler': True}
        batch = dataset.__getitem__(0, context=context)
        assert batch['p0_MR_T1_voxels'].shape == (60, 256, 192)
        assert batch['p1_MR_T1_voxels'].shape == (60, 256, 192)
...
def test_paired_list_h5_sampler_blocks():
    with tempfile.TemporaryDirectory() as path:
        ...
        assert np.abs(batch['MR_T1_voxels'][0] - batch['MR_T1_voxels'][1]).max() < 1e-6
